...
int main(void) {
	...
	else if (judge == 0) {//対応していない括弧があった時
		printf("出力：ERROR\"%c%c%c\"", *(p - 1), *p, *(p + 1));//ERRORした箇所の1文字前、その文字、1文字後を出力
	}
	return 0;
}
/*
...
入力：{{)
入力：((}
上記の出力はすべて「出力：ERROR parenthesis not closed」となった。
すべての()と{}が対応している場合
